...
const LoginForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.username.trim().match(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i)) {
      validationErrors.username = 'Please enter a valid email address';
    }

    if (!formData.password.match(/^(?=.*[A-Z])(?=.*\d)[A-Za-z\d@]{8,}$/)) {
      validationErrors.password =
        'Password must contain at least 8 characters, including one uppercase letter, one number,special characters(only): @';
    }
